...
import { VHR_TOP_HOLDERS_URL } from 'data/constants'
    ...
    if (topHolders.length === 0) {
      fetch(`api/cors?url=${VHR_TOP_HOLDERS_URL}`)
        ...
        .then((html) => {
          ...
          let items: Item[] = []
          let cur = []
          for (const [i, element] of tag.entries()) {
            cur[i % 4] = element.text
            if (i % 4 === 0 && i !== 0) {
              items[index] = {
                index: index,
                address: cur[1],
          ...
          topHolders.map((i, index) => {
            return (
              <QueryHandle
                ...
                callback={(data: any) => {
                  if (topHolders[index].org === false && isVerified(data.profiles.items[0]?.id)) {
                    topHolders[index].org = true
                    setTopHolders([...topHolders])
                  }

                  if (topHolders[index].handle !== data.profiles.items[0]?.handle) {
