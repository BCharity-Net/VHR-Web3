...
export const ProfileCell = (props: { value: string }) => {
  ...
  return (
    <a href={`/u/${user}`} target="_blank" rel="noopener noreferrer" className="text-brand-500">
      {user} {<ExternalLinkIcon className="w-4 h-4 inline-flex" />}
    </a>
  )
}

export const TotalHoursCell = (props: { value: { index: number; value: number }; vhr: string[] }) => {
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="text-brand-500">
      {value} {<ExternalLinkIcon className="w-4 h-4 inline-flex" />}
    </a>
  )
}

export const TotalGoodCell = (props: { value: { index: number; value: number }; good: string[] }) => {
  const decimals = 2
  const index = props.value.index
  const value = props.value.value
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="text-brand-500">
      {value.toFixed(decimals)} {<ExternalLinkIcon className="w-4 h-4 inline-flex" />}
    </a>
  )
}
